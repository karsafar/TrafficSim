...
for i = 1:sim.horizArm.numCars
    ...
    b_vec_e(:,1) = max(a_vec_e(:,1),a_vec_e(:,5));
    b_vec_e(:,2) = max(max(a_vec_e(:,2),a_vec_e(:,3)),a_vec_e(:,6));
    b_vec_e(:,3) = a_vec_e(:,4);
    b_vec_e(:,4) = a_vec_e(:,7);
...
%     title(ax2,'East Arm Acceleration trajectory','FontSize',16)
    ...
    a_aggerg_e = a_aggerg_e + (nansum(b_vec_e,1));
    a_aggerg_norn_e = a_aggerg_norn_e + (nansum(b_vec_e,1)/nIterations);
    
    %% North Arm
    a_vec_n = sim.vertArm.allCars(i).bbStore;
    a_vec_n(a_vec_n == -1) = NaN;
    
    b_vec_n(:,1) = max(a_vec_n(:,1),a_vec_n(:,5));
    b_vec_n(:,2) = max(max(a_vec_n(:,2),a_vec_n(:,3)),a_vec_n(:,6));
    b_vec_n(:,3) = a_vec_n(:,4);
    b_vec_n(:,4) = a_vec_n(:,7);
...
figure(4)
a_aggerg_norn_e = a_aggerg_norn_e/sim.horizArm.numCars;
c = categorical({'Follow Car','Junc Stop','Go Ahead','Emerg Brake'});
bar(c,a_aggerg_norn_e,'stacked');
...
figure(6)
a_aggerg_norn_n = a_aggerg_norn_n/sim.horizArm.numCars;
c = categorical({'Follow Car','Junc Stop','Go Ahead','Emerg Brake'});
bar(c,a_aggerg_norn_n,'stacked');
