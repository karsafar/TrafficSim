...
hold on
plot(ax,k,2*q*3100,'k-','LineWidth',2)
lgd = legend(ax,'Critical Density','Fundamental Diagram of Junction');
plot(ax,k,q*3100,'-','Color',[0.5 0.5 0.5 ],'LineWidth',2,'DisplayName',...
    'Findamental Diagram of Single Arm')
...
for i = 1:2
    ...
    plot(ax,density(1,:),numCrosses(i,:),colArray{i},'LineWidth',2,'DisplayName',dispname)
end
lgd.FontSize = 10;
...
plot(ax,density(1,:),numCrosses(3,:),'g-','LineWidth',2,'DisplayName',dispname)
lgd.FontSize = 10;
...
colArray = {'b-','c-'};
for i = 4:5
    dispname = sprintf('a_{IDM} = %s m/s^2',num2str(a_val(i-3)));
    plot(ax,density(1,:),numCrosses(i,:),colArray{i-3},'LineWidth',2,'DisplayName',dispname)
end
lgd.FontSize = 10;
