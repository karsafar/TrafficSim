...
fnm = sprintf('time-gap-variation/rand003-20min.mat')
...
mix = 3;% 1-mix arms; 2 - random arms; 3 - fixed arms
if mix == 1
    a_feas_min = random(pd,n,1); % %(m/s^2)
    a_feas_min = [a_feas_min; -9*ones(n,1)]; % (s)
elseif mix == 2
    a_feas_min = random(pd,2*n,1); % %(m/s^2)
elseif mix == 3
    a_feas_min = -9*ones(2*n,1); % (s)
end
