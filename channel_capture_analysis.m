...
for idx = 1:numel(data)
    numCars(1,idx)
    plot(data(idx).crossCount,'-b','LineWidth',1.5)
    axis([0 numel(data(idx).crossCount) 0 3])
    grid on
    xlabel('Number of Junction Crosses','FontSize',16)
    text(numel(data(idx).crossCount)/2,-0.1,'\uparrow East Arm Crosses','FontSize',16)
    text(numel(data(idx).crossCount)/2,1.1,'\downarrow North Arm Crosses','FontSize',16)
    ylim([-0.5,1.5])
    pause(0.25)
end
...
orderedPlatoons = NaN(length(data),1050);
...
figure
ax = axes;
barh(ax,numCars(1,:)',orderedPlatoons,'stacked')
xlabel('Crossing Platoon Sizes','FontSize',14)
ylabel('Numer of cars per arm','FontSize',14)
% xlim([0 1])
% ylim([2 n+1])
yticks(numCars(1,1):2:numCars(1,end))
%
